...
public class HeapsMergeKSortedLists {
    ...
    public static void mergeKSortedLists(int totalElements, List<Integer>... lists)
       ...
       // MinHeap<Element> minHeap = new MinHeap<>(Element.class, lists.length);
        ...
        // Put first elements of all list in Min Heap
        List<Integer> sortedList = new ArrayList<>();
        for (int i = 0; i < lists.length; i++) {
            List<Integer> list = lists[i];
            if (!list.isEmpty()) {
                minHeap.add(new Element(i, list.remove(0)));
            }
        }

        // Do while sortedList has size less than total elems
        //    remove the min priority and add to sorted list
        //    grab the list from which the min priority elem came
        //    add that to min heap
        while (sortedList.size() < totalElements) {
            Element element = (Element) minHeap.poll();
            sortedList.add(element.getValue());

            List<Integer> list = lists[element.getListIndex()];
            if (!list.isEmpty()) {
                minHeap.add(new Element(element.getListIndex(), list.remove(0)));
            }
        }
